...
@param[in]	crypt_info	encryption information
	...
	const crypt_info_t*	crypt_info)
{
	...
	rc = encryption_crypt(src_block, size,
				dst_block, &dst_len,
				(unsigned char*)(crypt_info->crypt_key), 16,
				aes_ctr_counter, MY_AES_BLOCK_SIZE,
				what | ENCRYPTION_FLAG_NOPAD,
				LOG_DEFAULT_ENCRYPTION_KEY,
				crypt_info->key_version);
	...
	ulint			space_id)
{
	...
	return (log_tmp_blocks_crypt(src_block, size, dst_block,
				ENCRYPTION_FLAG_ENCRYPT, offs, space_id, info));
}
	...
	ulint			space_id)
{
	...
	return (log_tmp_blocks_crypt(src_block, size, dst_block,
				ENCRYPTION_FLAG_DECRYPT, offs, space_id, info));
}
